...
This plugin is a fork of Also In This Collection ClassicPress Plugin by Jon Weatherhead.  
...
Manually embed a collection listing with the provided shortcode and widget. Override the collection template with a custom template.  

== How To Use ==  
Create new Collection terms from the Collection manager under Posts. Add posts to a collection either through the quick edit screen or through the full edit screen.  
...
To manually insert a specific collection, use the collection-slug attribute with the shortcode like:  
`[alsointhiscollection collection-slug="your-collection-slug"]`  
...
== Installation ==  
1. Unpack the plugin zip and upload the contents to the `/wp-content/plugins/` directory. Alternatively, upload the plugin zip via the install screen within the ClassicPress Plugins manager.  
2. Activate the plugin through the Plugins manager in ClassicPress.  
